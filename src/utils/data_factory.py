...
def get_data(data_type: str, device: torch.device) -> Dict:
    ...
    if data_type == "oscillatory":
        ...
        data = retrieve_simulated_data(data_dir="/gpfs/radev/project/krishnaswamy_smita/kx44/project/STAGED-Simulator/outputs/repressilator",sim_file="500_simulation_results.pkl")
        # Convert to device
        for key, value in data.items():
...
def get_available_data_types():
    ...
    return ['oscillatory', 'damped_oscillator', 'hex_grid', 'square_grid', 'sinusoidal', 'simulated','real'] 